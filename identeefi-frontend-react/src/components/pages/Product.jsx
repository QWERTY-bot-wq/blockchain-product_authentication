...
const Product = () => {
  ...
  const calculateWarrantyStatus = (
    manufacturingDateSeconds,
    warrantyMonths,
  ) => {
    if (!manufacturingDateSeconds || !warrantyMonths)
      return "Warranty info not available";
    ...
    const monthsLeft =
      (warrantyEnd.getFullYear() - today.getFullYear()) * 12 +
  ...
  const setData = (product) => {
    ...
    console.log("First timestamp:", rawHistory[0]?.timestamp);
    ...
    if (manuSeconds) {
      ...
      if (!isNaN(Number(manuSeconds))) {
        // Case 1: Unix timestamp
        formattedDate = dayjs(Number(manuSeconds) * 1000).format("DD/MM/YYYY");
      } else {
        // Case 2: Already a date string (YYYY-MM-DD)
        formattedDate = dayjs(manuSeconds).format("DD/MM/YYYY");
      }

      setManufacturingDate(formattedDate);
      setWarrantyStatus(calculateWarrantyStatus(manuSeconds, warrantyMonths));
    } else {
      setManufacturingDate("Invalid Date");
